...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      let maxLineWidth = Math.max(1, ...lineMetrics.map(m => m.width));
      if (maxLineWidth > maxTextWidth) {
        resolvedSize = Math.max(12, resolvedSize * (maxTextWidth / maxLineWidth));
        font = `${fontWeight} ${resolvedSize}px ${resolvedFamily}`;
        await waitForFonts(font);
        if (currentBuild !== buildId) return;
        offCtx.font = font;
        lineMetrics = lines.map(line => offCtx.measureText(line));
        maxLineWidth = Math.max(1, ...lineMetrics.map(m => m.width));
      }

      const ascent = Math.ceil(Math.max(...lineMetrics.map(m => m.actualBoundingBoxAscent || resolvedSize * 0.78)));
      const descent = Math.ceil(Math.max(...lineMetrics.map(m => m.actualBoundingBoxDescent || resolvedSize * 0.22)));
      const lineHeight = Math.round(resolvedSize * 1.15);
      const padding = Math.max(12, Math.ceil(resolvedSize * 0.08));
      const textWidth = Math.ceil(maxLineWidth);
      const textHeight = Math.ceil((lines.length - 1) * lineHeight + ascent + descent);
